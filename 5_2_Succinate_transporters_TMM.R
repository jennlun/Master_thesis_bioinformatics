...
bar_plot_relevant = function(all_means, rel_dev_treat, gene, tissue){
  ...
  # Calculate max value for scaling and round up to nearest 5
  max_value <- ceiling(max(rel_dev_treat[[gene]], na.rm = TRUE) / 5) * 5
  
  # Generate y-axis breaks rounded to nearest 5
  y_breaks <- seq(0, max_value, length.out = 6)
