...
bar_plot_relevant = function(all_means, rel_dev_treat, gene, tissue){
  ...
  # Find highest value
  high_val = ceiling(max(rel_dev_treat[[gene]]))
  
  # Make If-statement to determine max_value and y_breaks
  if (high_val < 5) {
    max_value <- 5
    y_breaks <- seq(0, max_value, by = 1)
  } else if (high_val > 5 & high_val < 30) {
    max_value <- ceiling(high_val / 5) * 5
    y_breaks <- seq(0, max_value, by = 5)
  } else if (high_val > 30 & high_val < 100) {
    max_value <- ceiling(high_val / 10) * 10
    y_breaks <- seq(0, max_value, by = 10)
  } else if (high_val > 100 & high_val < 200) {
    max_value <- 180
    y_breaks <- seq(0, max_value, by = 30)
  } else if (high_val > 200 & high_val < 300) {
    max_value <- 240
    y_breaks <- seq(0, max_value, by = 40)
  } else if (high_val >= 300) {
    max_value <- ceiling(high_val / 10) * 10
    y_breaks <- seq(0, max_value, by = 60)
  }
